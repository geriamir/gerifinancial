...
describe('RealEstateInvestment Model', () => {
  ...
  describe('generateAutoInstallments', () => {
    ...
    it('should preserve paid auto-generated installments on regeneration', async () => {
      const inv = await RealEstateInvestment.create({
        userId: user._id,
        name: 'Preserve Paid',
        type: 'rental',
      ...
      // Regenerate — paid mortgage should be preserved
      inv.generateAutoInstallments();

      const mortgages = inv.installments.filter(i => i.autoGenerated === 'mortgage');
      expect(mortgages).toHaveLength(2); // paid one + new one
      expect(mortgages.filter(m => m.status === 'paid')).toHaveLength(1);
    });
