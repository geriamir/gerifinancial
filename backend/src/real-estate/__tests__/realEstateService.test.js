...
describe('RealEstateService', () => {
  ...
  describe('create', () => {
    ...
    it('should auto-generate installments on creation with mortgage', async () => {
      const result = await realEstateService.create(user._id, {
        name: 'Auto Install Create',
        type: 'rental',
        currency: 'ILS',
        estimatedCurrentValue: 1000000,
        mortgagePercentage: 60,
        purchaseTaxRate: 8
      });

      const mortgage = result.installments.find(i => i.autoGenerated === 'mortgage');
      const finalPay = result.installments.find(i => i.autoGenerated === 'final-payment');
      expect(mortgage).toBeTruthy();
      expect(mortgage.amount).toBe(600000);
      expect(finalPay).toBeTruthy();
      expect(finalPay.amount).toBe(480000); // 400K equity + 80K tax
    });
  });
  ...
  describe('update', () => {
    ...
    it('should regenerate auto-installments when mortgage/value/tax changes', async () => {
      const inv = await realEstateService.create(user._id, {
        name: 'Regen Test',
        type: 'rental',
        currency: 'ILS',
      ...
      // Verify initial auto-installments
      expect(inv.installments.find(i => i.autoGenerated === 'mortgage').amount).toBe(500000);

      // Update mortgage percentage
      const updated = await realEstateService.update(inv._id, user._id, {
        mortgagePercentage: 75
      });

      const mortgage = updated.installments.find(i => i.autoGenerated === 'mortgage' && i.status !== 'paid');
