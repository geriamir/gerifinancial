...
realEstateInvestmentSchema.methods.generateAutoInstallments = function() {
  ...
  const hasPaidMortgage = this.installments.some(
    i => i.autoGenerated === 'mortgage' && i.status === 'paid'
  );
  const hasPaidFinalPayment = this.installments.some(
    i => i.autoGenerated === 'final-payment' && i.status === 'paid'
  );
  ...
  // Only generate new mortgage if there isn't a paid one already
  if (mortgageAmount > 0 && !hasPaidMortgage) {
  ...
  // Sum investment-type installments in the same currency (excluding final-payment)
  const investmentInstallmentsTotal = this.installments
    .filter(
      i => i.installmentType === 'investment' &&
        i.autoGenerated !== 'final-payment' &&
        (!i.currency || i.currency === currency)
    )
  ...
  // Only generate new final-payment if there isn't a paid one already
  if (finalPaymentAmount > 0 && !hasPaidFinalPayment) {
    this.installments.push({
