...
class BudgetService {
  ...
  async getBudgetVsActual(userId, type, period) {
    try {
      ...
      switch (type) {
        ...
        case 'project':
          throw new Error('Project budget operations should be handled by the project-budgets subsystem');
  ...
  async getUserBudgets(userId, year) {
    try {
      const [monthlyBudgets, yearlyBudget] = await Promise.all([
      ...
      return {
        ...
        yearly: yearlyBudget,
        projects: [] // Project budget operations moved to project-budgets subsystem
      };
    } catch (error) {
  ...
  async getDashboardOverview(userId) {
    try {
      ...
      const currentMonthBudget = await this.getMonthlyBudget(userId, currentYear, currentMonth);
      const projectStats = await projectBudgetService.getProjectBudgetStats(userId);

      return {
        currentMonth: currentMonthBudget,
        activeProjects: projectStats.activeProjects,
        upcomingProjects: projectStats.upcomingProjects,
        totalActiveProjectBudget: projectStats.totalActiveProjectBudget
      };
    } catch (error) {
  ...
  async getActualAmountsForMonthWithExclusions(userId, year, month) {
    try {
      ...
      const exclusionFilter = { excludeFromBudgetCalculation: { $ne: true } };
      ...
      // Adjust for salary arriving up to 5 days before month start
      transactions = await adjustForSalaryEarlyPayment(transactions, userId, year, month, exclusionFilter);
